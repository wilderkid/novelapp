  ...
  <div class="resource-sidebar">
    ...
    >
      <div class="category-header" @click="handleCategoryClick(resource)">
        <el-icon
          v-if="resource.type !== 'worldview'"
          class="expand-icon"
          :class="{ expanded: expandedState[resource.type] }"
        >
          <ArrowRight />
        </el-icon>
        <span class="category-name"
          >{{ resource.emoji }} {{ resource.name }}</span
        >
        <div class="actions" v-if="resource.type !== 'worldview'">
          <el-icon
            class="action-icon"
            @click.stop="emit('open-summary-modal', resource.type)"
            ><InfoFilled
          /></el-icon>
          <el-icon
            class="action-icon"
            @click.stop="emit('open-add-modal', resource.type)"
            ><Plus
          /></el-icon>
        </div>
      </div>
