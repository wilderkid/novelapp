...
export const processOpenAIUrl = (userProvidedUrl) => {
  ...
  if (userProvidedUrl.endsWith("#")) {
    ...
    const modelsEndpoint =
      chatIndex !== -1 ? baseUrl.substring(0, chatIndex) + "/models" : "";
...
export const checkApiKey = async (provider) => {
  try {
    const {
      chatCompletionEndpoint,
      modelsEndpoint,
      isManualModelEntryRequired,
    } = processOpenAIUrl(provider.base_url);
...
export const getModelsFromProvider = async (provider) => {
  const { modelsEndpoint, isManualModelEntryRequired } = processOpenAIUrl(
    provider.base_url,
  );
...
export const reorderProviders = (providerIds) => {
  return axios.put(`${API_URL}/ai-providers/reorder`, {
