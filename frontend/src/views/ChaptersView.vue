              ...
              <el-dropdown trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item @click="handleNewChapter(null)" :disabled="volumes.length === 0">新建章节</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </div>
                ...
                <div class="tree-node" :data-id="data.id">
                  ...
                  <div class="node-actions" v-if="!data.isFolder || data.id !== 'root'">
                    ...
                    <template v-if="data.type === 'volume'">
                      ...
                      <el-button size="small" text @click.stop="handleNewChapter(data)">
                        <el-icon><Plus /></el-icon>
                      </el-button>
                    </template>
                    <!-- 章节操作按钮 -->
      ...
      >
        <el-form :model="chapterForm" label-width="80px">
          ...
          <el-form-item label="所属分卷">
            <el-select v-model="chapterForm.volume_id" placeholder="请选择分卷" style="width: 100%" :disabled="!isVolumeSelectable">
              <el-option
      ...
      >
        <div v-if="currentVolume" class="volume-detail">
          <div class="volume-header">
            ...
            <el-tag type="info" size="small">第{{ currentVolume.order }}卷</el-tag>
          </div>
          <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="章节数量">
              <el-tag type="success">{{ currentVolume.chapterCount }} 章</el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="总字数">
              <el-tag type="warning">{{ formatWordCount(currentVolume.wordCount) }}</el-tag>
            </el-descriptions-item>
          </el-descriptions>
  ...
  volumes.value.forEach(volume => {
    ...
    const volumeChapters = chapters.value
      .filter(chapter => {
        ...
        console.log(`章节 ${chapter.id} volumeId/volume_id: ${chapterVolumeId}, 分卷ID: ${volume.id}, 匹配: ${match}`)
...
const openChapter = async (chapter) => {
  try {
    ...
    ElMessage.error('加载章节数据失败: ' + (error.response?.data?.message || error.message))
...
const loadVolumesAndChapters = async () => {
  ...
  try {
    ...
    // 加载分卷数据
    console.log('加载分卷数据...')
    const volumesData = await volumeService.getVolumes(projectStore.currentProject.id)
...
const getVolumeChapters = (volumeId) => {
  return chapters.value.filter(chapter => {
    ...
    return {
      ...chapter,
      wordCount: chapter.wordCount || chapter.word_count || calculatedWordCount
    }
  })
}
...
const saveVolume = async () => {
  ...
  try {
    ...
    ElMessage.error('保存分卷失败: ' + (error.response?.data?.message || error.message))
  }
}
...
const confirmDeleteVolume = async (volume) => {
  try {
    ...
    if (error !== 'cancel') {
      ...
      ElMessage.error('删除分卷失败: ' + (error.response?.data?.message || error.message))
    }
  }
}
...
const saveChapterForm = async () => {
  ...
  try {
    ...
    if (isEditingChapter.value) {
      ...
      const updatedChapter = await chapterService.saveChapter(updatedChapterData)
...
const confirmDeleteChapter = async (chapter) => {
  try {
    ...
    if (error !== 'cancel') {
      ...
      ElMessage.error('删除章节失败: ' + (error.response?.data?.message || error.message))
    }
  }
}
