<template>
  <div class="resource-view-container">
        ...
        <div class="editor-area">
          <div v-if="currentResource" class="resource-editor">
            <div class="editor-header">
              <el-input
                v-if="currentResource.type !== 'worldview'"
                v-model="currentResource.item.name"
                placeholder="资源名称"
                class="title-input"
              />
              <h3 v-else>世界观</h3>
      ...
      <el-table :data="resourceList" style="width: 100%" max-height="400">
        ...
        <el-table-column label="操作" width="120">
          <template #default="scope">
            <el-button size="small" @click="editResourceFromModal(scope.row)"
              >编辑</el-button
            >
            <el-button
              size="small"
              type="danger"
              @click="confirmDeleteResource(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    <!-- 新增条目弹窗 -->
    <el-dialog
      v-model="addItemModalVisible"
      :title="'新增' + resourceTypeInfo.name"
      width="30%"
    >
      <el-form>
...
const resourceTypeInfo = computed(
  () => resourceTypeMap[currentOpenResourceType.value] || { name: "" },
);
...
const addNewItem = async () => {
  ...
  try {
    await service.create(projectStore.currentProject.id, {
    ...
    .then(async () => {
      ...
      try {
        ...
        if (
          ...
          currentResource.value.item.id === item.id
        ) {
