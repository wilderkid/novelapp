    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane name="genres" label="小说类型管理">
        <div class="genres-management">
          <el-button type="primary" @click="showGenreDialog = true" style="margin-bottom: 15px">
          ...
          <el-table :data="genres" style="width: 100%">
            ...
            <el-table-column label="操作" width="200">
              <template #default="scope">
                <el-button size="small" @click="editGenre(scope.row)">编辑</el-button>
                <el-button size="small" type="danger" @click="deleteGenre(scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
      ...
      <el-tab-pane name="chat" label="AI对话默认设置">
        <el-form :model="chatDefaults" label-width="120px" class="settings-form">
          <el-form-item label="默认AI模型">
            <el-select v-model="chatDefaults.aiModelId" placeholder="选择默认AI模型" style="width: 100%">
              <el-option label="无默认模型" :value="null" />
              <el-option-group v-for="(models, providerName) in groupedAiModels" :key="providerName" :label="providerName">
                <el-option v-for="model in models" :key="model.id" :label="model.name" :value="model.id" />
              </el-option-group>
            </el-select>
          </el-form-item>
          <el-form-item label="默认提示词">
            <el-select v-model="chatDefaults.promptTemplateId" placeholder="选择默认提示词" style="width: 100%">
          ...
          <el-form-item label="默认温度">
            <el-input-number v-model="chatDefaults.temperature" :min="0" :max="1" :step="0.1" :precision="1" />
            <div class="form-item-tip">控制输出的随机性，0-1之间</div>
          </el-form-item>
          <el-form-item label="默认最大Token">
            <el-input-number v-model="chatDefaults.maxTokens" :min="100" :max="1000000" :step="100" />
            <div class="form-item-tip">限制AI回复的最大长度</div>
          </el-form-item>
          <el-form-item label="默认记忆轮数">
            <el-input-number v-model="chatDefaults.memoryRounds" :min="1" :max="200" :step="1" />
            <div class="form-item-tip">保留最近N轮对话作为上下文</div>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="saveChatDefaults">保存AI对话默认设置</el-button>
          </el-form-item>
        </el-form>
      </el-tab-pane>

      <el-tab-pane name="assistant" label="创作助手默认设置">
        <el-form :model="assistantDefaults" label-width="120px" class="settings-form">
          <el-form-item label="默认AI模型">
            <el-select v-model="assistantDefaults.aiModelId" placeholder="选择默认AI模型" style="width: 100%">
              <el-option label="无默认模型" :value="null" />
              <el-option-group v-for="(models, providerName) in groupedAiModels" :key="providerName" :label="providerName">
                <el-option v-for="model in models" :key="model.id" :label="model.name" :value="model.id" />
              </el-option-group>
          ...
          <el-form-item label="默认提示词">
            <el-select v-model="assistantDefaults.promptTemplateId" placeholder="选择默认提示词" style="width: 100%">
              <el-option label="无默认提示词" :value="null" />
              <el-option v-for="template in promptTemplates" :key="template.id" :label="template.name" :value="template.id" />
            </el-select>
          </el-form-item>
          <el-form-item label="默认温度">
            <el-input-number v-model="assistantDefaults.temperature" :min="0" :max="1" :step="0.1" :precision="1" />
            <div class="form-item-tip">控制输出的随机性，0-1之间</div>
          </el-form-item>
          <el-form-item label="默认最大Token">
            <el-input-number v-model="assistantDefaults.maxTokens" :min="100" :max="1000000" :step="100" />
            <div class="form-item-tip">限制AI回复的最大长度</div>
          </el-form-item>
          <el-form-item label="默认记忆轮数">
            <el-input-number v-model="assistantDefaults.memoryRounds" :min="1" :max="200" :step="1" />
            <div class="form-item-tip">保留最近N轮对话作为上下文</div>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="saveAssistantDefaults">保存创作助手默认设置</el-button>
          </el-form-item>
        </el-form>
      </el-tab-pane>
    </el-tabs>

    <el-dialog v-model="showGenreDialog" :title="genreForm.id ? '编辑类型' : '新增类型'" width="400px">
...
const deleteGenre = async (genre) => {
  try {
    await ElMessageBox.confirm(`确定要删除类型"${genre.name}"吗？`, '删除确认', {
