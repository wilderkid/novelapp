...
2.  **创建数据库**:
    ...
    ```
3.  **初始化表结构**:
    -   使用 `psql` 工具连接到 `storyforge` 数据库，并执行 `database/init.sql` 脚本。
    ```bash
    # psql -U <您的用户名> -d storyforge -f database/init.sql
    # 示例:
    psql -U postgres -d storyforge -f ..\database\init.sql
    ```
    -   执行此命令时，请确保路径正确。如果您的 `database` 目录在上一级，应使用 `..\database\init.sql`。
